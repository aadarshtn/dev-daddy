...
router.delete('/experience/:exp_id', auth, async (req,res) => {
    try {
        ...
        const removingExpIndex = profile.experience.map(exp => exp.id).indexOf(req.params.exp_id);
...
router.delete('/education/:edu_id', auth, async (req,res) => {
    try {
        ...
        const removingEduIndex = profile.education.map(edu => edu.id).indexOf(req.params.edu_id);
    ...
    try {
        ...
        request(options, (error, response, body) => {
            ...
            if(response.statusCode !== 200) {
                console.log(body)
                return res.status(404).json({ msg: "Requested user not found in github" });
            }
